...
const Add = ({ token }) => {
  ...
  const [subCategory, setSubCategory] = useState("Oil");
  const [outOfStock, setOutOfStock] = useState(false);

  const userGroup = localStorage.getItem('userGroup'); // 'admin' sau 'artist'
  ...
  useEffect(() => {
    const fetchArtists = async () => {
      ...
      try {
        const res = await axios.get(backendUrl + '/api/user/artists', {
          headers: { token }
        });
        ...
        console.log(err);
        toast.error("Failed to load artist list");
      }
    };
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      formData.append("artistId", userGroup === 'admin' ? selectedArtist : getUserIdFromToken(token));
...
const getUserIdFromToken = (token) => {
  try {
    const decoded = jwtDecode(token);  // token trebuie să fie un string valid
